...
class GraphicsDevice
{
    ...
    IDCompositionTarget target; IDCompositionVisual2 root; IDCompositionVisual2[3] rects; IDCompositionSurface surface; IDCompositionScaleTransform scaler;
    ...
    this(HWND hwTarget)
    {
        ...
        immutable cp = D2D1_CREATION_PROPERTIES(D2D1_THREADING_MODE_SINGLE_THREADED, D2D1_DEBUG_LEVEL_ERROR, D2D1_DEVICE_CONTEXT_OPTIONS_NONE);
        ...
        foreach(ref e; this.rects) this.compDevice.CreateVisual(&e).checkFailure();
        ...
        this.compDevice.CreateSurface(1, 1, DXGI_FORMAT_B8G8R8A8_UNORM, DXGI_ALPHA_MODE_PREMULTIPLIED, &this.surface).checkFailure();
        {
            ID2D1DeviceContext con; POINT offs;
            this.surface.BeginDraw(null, &IID_ID2D1DeviceContext, cast(void**)&con, &offs).checkFailure();
            scope(exit) { this.surface.EndDraw().checkFailure(); con.Release(); }
            ...
            con.SetTransform(cast(D2D_MATRIX_3X2_F*)&Transform); con.Clear(&ClearColor);
        }
        this.compDevice.CreateScaleTransform(&this.scaler).checkFailure();
        this.scaler.SetScaleX(100).checkFailure(); this.scaler.SetScaleY(100).checkFailure();
        foreach(i, e; this.rects)
        {
            e.SetContent(this.surface).checkFailure();
            e.SetTransform(this.scaler).checkFailure();
            e.SetOffsetX(16 + i * 16).checkFailure(); e.SetOffsetY(16 + i * 16).checkFailure();
        }
    ...
    ~this()
    {
        this.scaler.Release(); this.surface.Release();
        foreach(ref e; this.rects) e.Release();
...
extern(Windows) int WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
    ...
    try
    {
        ...
        auto hw = CreateWindowExA(WS_EX_NOREDIRECTIONBITMAP | WS_EX_APPWINDOW, cast(char*)wndclass, "dcomp example".toMBSz,
